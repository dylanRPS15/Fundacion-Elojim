...
export default function RefuerzoEscolarPage() {
  ...
  const fetchRegistros = async () => {
    try {
      const res = await fetch("/api/registro/refuerzo-escolar");
      if (res.ok) {
  ...
  const registrosFiltrados = registros.filter((registro) => {
    const matchesBusqueda =
      ...
      registro.cursoGrado?.toLowerCase().includes(busqueda.toLowerCase());
    ...
    <ContentLayout title="Programa de Jornadas de Refuerzo Escolar">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>Refuerzo Escolar</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <div className="relative max-w-md mt-8">
        <input
          ...
          className="w-full rounded-lg border border-gray-300 bg-white py-2 pl-10 pr-4 text-sm shadow-sm placeholder:text-gray-400 focus:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-300"
        />
      ...
      {/* Filtros */}
      <div className="mt-10 mb-8 bg-white shadow-md rounded-xl border border-orange-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <select
            ...
            className="border border-gray-300 p-2 rounded-lg text-sm focus:ring-2 focus:ring-amber-300"
          >
          ...
          <input
            ...
            className="border border-gray-300 p-2 rounded-lg text-sm focus:ring-2 focus:ring-orange-300"
          />
        </div>
      </div>
          ...
          <tbody>
            {registrosFiltrados.map((registro) => (
              ...
              >
                ...
                <td className="py-2 px-4 text-gray-700">{registro.cursoGrado}</td>
                <td className="py-2 px-4 flex gap-2">
                  <button
                    onClick={() => handleViewRecord(registro)}
                    className="flex items-center gap-1 bg-amber-600 hover:bg-amber-700 text-white text-xs px-3 py-1.5 rounded-lg transition"
                  >
                    <svg
