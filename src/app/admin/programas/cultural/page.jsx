...
export default function CulturalPage() {
  ...
  const fetchRegistros = async () => {
    try {
      const res = await fetch("/api/registro/cultural");
      if (res.ok) {
        const data = await res.json();
        // Opcional: ordenar por nombre y documento igual que en SoftwareFactoryPage
        const sorted = data.sort((a, b) => {
          const nameCompare = a.nombreCompleto.localeCompare(b.nombreCompleto);
          if (nameCompare !== 0) return nameCompare;
          return a.documentoIdentidad?.localeCompare(b.documentoIdentidad || "") || 0;
        });
        setRegistros(sorted);
      } else {
  ...
  const registrosFiltrados = registros.filter((registro) => {
    const matchesBusqueda =
      ...
      registro.documentoIdentidad?.toLowerCase().includes(busqueda.toLowerCase());
  ...
  return (
    <ProtectedAdmin>
    <ContentLayout title="Programa Cultural">
      <Breadcrumb>
        <BreadcrumbList>
          <BreadcrumbItem>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>Cultural</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <div className="relative max-w-md mt-8">
        <input
          ...
          className="w-full rounded-lg border border-gray-300 bg-white py-2 pl-10 pr-4 text-sm shadow-sm placeholder:text-gray-400 focus:border-purple-400 focus:outline-none focus:ring-2 focus:ring-purple-300"
        />
      ...
      {/* Filtros */}
      <div className="mt-10 mb-8 bg-white shadow-md rounded-xl border border-purple-400 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <select
            ...
            className="border border-gray-300 p-2 rounded-lg text-sm focus:ring-2 focus:ring-purple-500"
          >
