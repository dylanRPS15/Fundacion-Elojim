...
export default function EconomiaPlateadaPage() {
  ...
  const fetchRegistros = async () => {
    try {
      const res = await fetch("/api/registro/economia-plateada");
      if (res.ok) {
        const data = await res.json();
        // Opcional: ordenar por nombre y documento como en el otro ejemplo
  ...
  // Filtrado con todos los filtros aplicados
  const registrosFiltrados = registros.filter((registro) => {
    ...
    const matchesEdad = edadExacta !== "" ? registro.edad === parseInt(edadExacta) : true;
    ...
    <ContentLayout title="Programa de Economía Plateada">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>Economía Plateada</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <div className="relative max-w-md mt-8">
        <input
          ...
          className="w-full rounded-lg border border-gray-300 bg-white py-2 pl-10 pr-4 text-sm shadow-sm placeholder:text-gray-400 focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-300"
        />
      ...
      {/* Filtros */}
      <div className="mt-10 mb-8 bg-white shadow-md rounded-xl border border-slate-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <select
            ...
            className="border border-gray-300 p-2 rounded-lg text-sm focus:ring-2 focus:ring-slate-300"
          >
              ...
              >
                ...
                <td className="py-2 px-4 flex gap-2">
                  <button
                    ...
                    className="flex items-center gap-1 bg-slate-600 hover:bg-slate-700 text-white text-xs px-3 py-1.5 rounded-lg transition"
                  >
                    <svg
