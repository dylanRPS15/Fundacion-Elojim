...
export default function EconomiaPlateadaEventoPage() {
  ...
  // 🔹 Info del programa
  const program = programs.find((p) => p.id === "economia-plateada");
  const Icon = program.icon;

  return (
    <ProtectedAdmin>
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50 py-12 px-6 flex flex-col items-center relative overflow-hidden">
        ...
        >
          ...
          <p className="text-gray-500 text-sm mt-2 max-w-lg">
            Registra un nuevo evento del programa{" "}
            <b>{program.title}</b> y gestiona toda la información desde el panel administrativo.
          </p>
        </motion.div>
